...
public:
    ...
    CallbackReturn on_configure(const rclcpp_lifecycle::State &) override {
        ...
        // -- 应用层目标指令订阅 (来自 app_motioncontrol_node, 尚未建立) --
...
private:
    ...
    void load_parameters() {
        ...
        if (alloc_vec_.size() != EXPECTED_ALLOC_SIZE) {
            RCLCPP_WARN(get_logger(),
                "[MC] 分配矩阵元素数 = %zu (期望%zu). 使用默认值.", alloc_vec_.size(), EXPECTED_ALLOC_SIZE);
            alloc_vec_.assign(EXPECTED_ALLOC_SIZE, 0.0);
        }
        ...
        control_period_s_ = 1.0 / this->get_parameter("control_rate_hz").as_double();
    ...
    void control_loop() {
        ...
        double dt_cmd = (this->now() - last_cmd).seconds();
    ...
    void publish_servo_commands(const TargetSetpoint & /*target*/,
        ...
        // TODO: 当 enable_servo=true 时在此实现舵机分配逻辑
    ...
    double estimate_yaw_rate_from_target(const TargetSetpoint & target) {
        // 简易实现: 取当前目标与上一帧目标之差除以控制周期
        static double prev_yaw_target = target.yaw;
        double rate = wrap_angle(target.yaw - prev_yaw_target) / control_period_s_;
        prev_yaw_target = target.yaw;
        // 低通滤波抑制阶跃噪声
        static double filtered_rate = 0.0;
        filtered_rate = 0.6 * filtered_rate + 0.4 * rate;
    ...
    // -- 控制分配 --
    std::vector<double> alloc_vec_;  // 6×7 = 42 元素, 行优先
